...
class Admin extends user
{
  ...
  public function editProduct($productId, $newName, $newPrice, $newQuantity, $newCategory, $newImg)
{
    $conn = Database::getInstance()->getConnection();
    try {
        $stmt = $conn->prepare("UPDATE products SET name = ?, price = ?, quantity = ?, category = ?, img = ? WHERE ID = ?");
        $stmt->execute([$newName, $newPrice, $newQuantity, $newCategory, $newImg, $productId]);
  ...
  public function manageInventory()
  {
    ...
    try {
      ...
      foreach ($products as $product) {
        $output .= "- {$product['product_id']} | {$product['name']} | {$product['price']} EGP\n";
      }
      return $output;
    } catch (PDOException $e) {
      return "Error: " . $e->getMessage();
